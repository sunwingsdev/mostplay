...
const getFavoritesPoster = async (req, res) => {
  try {
    ...
    return sendResponse(
      ...
      200,
      true,
      "Favorites Poster retrieved successfully",
      poster
    );
  } catch (error) {
...
const updateFavoritesPoster = async (req, res) => {
  try {
    ...
    return sendResponse(
      ...
      200,
      true,
      "Favorites Poster updated successfully",
      poster
    );
  } catch (error) {
    return sendResponse(res, 500, false, "Server error: " + error.message);
...
const deleteFavoritesPosterImage = async (req, res) => {
  try {
    ...
    poster.images = poster.images.filter(
      (url) => url !== decodeURIComponent(imageUrl)
    );
    await poster.save();
...
const updateFeaturedGames = async (req, res) => {
  try {
    ...
    return sendResponse(
      ...
      200,
      true,
      "Featured Games updated successfully",
      games
    );
  } catch (error) {
    return sendResponse(res, 500, false, "Server error: " + error.message);
...
const updateFeaturedGameItem = async (req, res) => {
  try {
    ...
    const itemIndex = games.items.findIndex(
      (item) => item._id.toString() === itemId
    );
    if (itemIndex === -1) {
      return sendResponse(res, 404, false, "Game item not found");
    }
    games.items[itemIndex] = {
      ...games.items[itemIndex],
      image,
      title,
      titleBD,
...
const removeFeaturedGameItem = async (req, res) => {
  try {
    ...
    return sendResponse(
      ...
      200,
      true,
      "Game item removed successfully",
      games
    );
  } catch (error) {
    return sendResponse(res, 500, false, "Server error: " + error.message);
